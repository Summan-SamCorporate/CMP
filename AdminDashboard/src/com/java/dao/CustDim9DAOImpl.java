...
public class CustDim9DAOImpl implements CustDim9DAO {
	@Override
	...
	public List<CustDim9> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim9> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim9.class);
	...
	public List<CustDim9> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim9> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim9.class);
			crit.add(Restrictions.eq("application_no",application_no));
			crit.addOrder(Order.asc("custdim9_code"));
