...
public class CustDim12DAOImpl implements CustDim12DAO {
	@Override
	...
	public List<CustDim12> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim12> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim12.class);
	...
	public List<CustDim12> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim12> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim12.class);
			crit.add(Restrictions.eq("application_no",application_no));
			
			crit.addOrder(Order.asc("custdim12_code"));
