...
public class CustDim8DAOImpl implements CustDim8DAO {
	@Override
	...
	public List<CustDim8> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim8> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim8.class);
	...
	public List<CustDim8> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim8> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim8.class);
			crit.add(Restrictions.eq("application_no",application_no));
			 crit.addOrder(Order.asc("custdim8_code"));
