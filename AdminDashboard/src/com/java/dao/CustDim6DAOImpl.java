...
public class CustDim6DAOImpl implements CustDim6DAO {
	@Override
	...
	public List<CustDim6> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim6> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim6.class);
	...
	public List<CustDim6> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim6> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim6.class);
			crit.add(Restrictions.eq("application_no",application_no));
			   crit.addOrder(Order.asc("custdim6_code"));
