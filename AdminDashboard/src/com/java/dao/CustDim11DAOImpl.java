...
public class CustDim11DAOImpl implements CustDim11DAO {
	...
	public List<CustDim11> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim11> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim11.class);
	...
	public List<CustDim11> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim11> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim11.class);
			crit.add(Restrictions.eq("application_no",application_no));
			crit.addOrder(Order.asc("custdim11_code"));
