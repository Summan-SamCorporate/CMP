...
public class CustDim1DAOImpl implements CustDim1DAO {
	@Override
	...
	public List<CustDim1> list(String application_no) {
		...
		Transaction tx = null;
		List<CustDim1> custdim = null;
		try {
			tx = session.beginTransaction();

			Criteria crit = session.createCriteria(CustDim1.class);
	...
	public List<CustDim1> getAll(String application_no, int start_index, int limit,List<Filter> filter) {
		Session session = HibernateUtil.openSession();
		Transaction tx = null;
		List<CustDim1> dim = null;
		try {
			tx = session.beginTransaction();
			Criteria crit = session.createCriteria(CustDim1.class);
			crit.add(Restrictions.eq("application_no",application_no));
			crit.addOrder(Order.asc("custdim1_code"));
